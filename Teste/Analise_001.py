...
# Parâmetros de tempo ativo
threshold_params = {"valor": 45.0, "tempo_ativo": 0.0}
threshold_line = {"obj": None}
...
def exportar_analise():
    ...
    try:
        with pd.ExcelWriter(fn, engine='openpyxl') as writer:
            ...
            # Adiciona planilha com métricas de tempo ativo
            metricas_df = pd.DataFrame({
                'Métrica': ['Tempo Total (s)', 'Tempo Ativo (s)', 'Threshold (mmHg)', '% Ativo'],
                'Valor': [
                    df[TIME_COL].iloc[-1] - df[TIME_COL].iloc[0] if len(df) > 0 else 0,
                    threshold_params["tempo_ativo"],
                    threshold_params["valor"],
                    (threshold_params["tempo_ativo"] / (df[TIME_COL].iloc[-1] - df[TIME_COL].iloc[0]) * 100) if len(df) > 1 and (df[TIME_COL].iloc[-1] - df[TIME_COL].iloc[0]) > 0 else 0
                ]
            })
            metricas_df.to_excel(writer, sheet_name='Métricas de Manobra', index=False)
...
# ===================== FUNÇÕES DE PLOTAGEM =====================
def calcular_tempo_ativo(t, x, threshold):
    """Calcula o tempo total em que o sinal está acima do threshold."""
    if len(t) < 2:
        return 0.0
    
    # Identifica regiões acima do threshold
    acima_threshold = x >= threshold
    
    # Calcula tempo total usando os intervalos de tempo
    dt = np.diff(t)
    # Para cada intervalo, se QUALQUER das duas amostras está acima, conta o intervalo
    tempo_ativo = 0.0
    for i in range(len(dt)):
        if acima_threshold[i] or acima_threshold[i+1]:
            tempo_ativo += dt[i]
    
    return tempo_ativo

def desenhar_threshold_line(threshold):
    """Desenha linha horizontal do threshold no gráfico."""
    if threshold_line["obj"] is not None:
        try:
            threshold_line["obj"].remove()
        except Exception:
            pass
    
    if not df.empty:
        threshold_line["obj"] = ax_sinal.axhline(y=threshold, color='orange', linestyle='--', linewidth=2, alpha=0.7, label='Tempo Ativo')
...
def recomputa_e_desenha_picos(df_cur, atualizar_metricas=True):
    ...
    ax_sinal.plot(tp, xp, "o", label="picos")
    
    # Desenha linha de threshold e calcula tempo ativo
    threshold = threshold_params["valor"]
    desenhar_threshold_line(threshold)
    tempo_ativo = calcular_tempo_ativo(t, x, threshold)
    threshold_params["tempo_ativo"] = tempo_ativo
    tempo_total = t[-1] - t[0] if len(t) > 0 else 0.0
    lbl_tempo_total_corte.configure(text=f"Tempo Total: {tempo_total:.2f} s")
    lbl_tempo_ativo.configure(text=f"Tempo Ativo: {tempo_ativo:.2f} s")
    
    ax_sinal.legend(loc="upper right"); canvas_sinal.draw_idle()
    ...
    if atualizar_metricas:
        ...
        lbl_npicos.configure(text=f"Picos: {n_peaks}"); lbl_dur.configure(text=f"Δt (1º→último): {dur_peaks:.2f} s"); lbl_freq.configure(text=f"Frequência: {freq_hz:.2f} Hz  ({60.0*freq_hz:.1f} BPM)")
...
frame_plot.grid_columnconfigure(0, weight=0)  # Slider vertical threshold
frame_plot.grid_columnconfigure(1, weight=1)  # Gráficos
...
# Frame para slider vertical de threshold (ao lado do gráfico de sinal)
frame_threshold_slider = ctk.CTkFrame(frame_plot, fg_color="transparent")
frame_threshold_slider.grid(row=0, column=0, sticky="ns", padx=(5,0), pady=5)
frame_threshold_slider.grid_rowconfigure(0, weight=0)  # Label
frame_threshold_slider.grid_rowconfigure(1, weight=1)  # Slider
frame_threshold_slider.grid_rowconfigure(2, weight=0)  # Valor

lbl_threshold_titulo = ctk.CTkLabel(frame_threshold_slider, text="Threshold\nAtivo", font=ctk.CTkFont(size=11, weight="bold"))
lbl_threshold_titulo.grid(row=0, column=0, pady=(5,10))

s_threshold = ctk.CTkSlider(frame_threshold_slider, from_=35, to=90, orientation="vertical", height=300)
s_threshold.set(45.0)
s_threshold.grid(row=1, column=0, pady=5)

lbl_threshold_val = ctk.CTkLabel(frame_threshold_slider, text=f"{45.0:.1f}\nmmHg", font=ctk.CTkFont(size=11))
lbl_threshold_val.grid(row=2, column=0, pady=(10,5))

fig_sinal, ax_sinal = plt.subplots(); canvas_sinal = FigureCanvasTkAgg(fig_sinal, master=frame_plot)
canvas_sinal.get_tk_widget().grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
toolbar_sinal = NavigationToolbar2Tk(canvas_sinal, frame_plot, pack_toolbar=False)
toolbar_sinal.update(); toolbar_sinal.grid(row=1, column=1, sticky="ew", padx=5)

fig_fft, ax_fft = plt.subplots(); canvas_fft = FigureCanvasTkAgg(fig_fft, master=frame_plot)
canvas_fft.get_tk_widget().grid(row=2, column=1, sticky="nsew", padx=5, pady=5)
toolbar_fft = NavigationToolbar2Tk(canvas_fft, frame_plot, pack_toolbar=False)
toolbar_fft.update(); toolbar_fft.grid(row=3, column=1, sticky="ew", padx=5)

frame_sliders_tempo = ctk.CTkFrame(frame_plot, fg_color="transparent")
frame_sliders_tempo.grid(row=4, column=1, sticky="ew", padx=5, pady=5)
...
# Callback para o slider de threshold
def on_threshold_change(v):
    v = float(v)
    threshold_params["valor"] = v
    lbl_threshold_val.configure(text=f"{v:.1f}\nmmHg")
    schedule_update_picos()

s_threshold.configure(command=on_threshold_change)
...
def resetar():
    ...
    state["t0"], state["t1"] = lo_n, hi_n; lbl_inicio.configure(text=f"Início: {lo_n:.3f} s"); lbl_fim.configure(text=f"Fim: {hi_n:.3f} s")
    have_cut["v"] = False
    lbl_npicos.configure(text="Picos: —"); lbl_dur.configure(text="Δt (1º→último): —"); lbl_freq.configure(text="Frequência: —")
    lbl_tempo_total_corte.configure(text="Tempo Total: 0.00 s")
    lbl_tempo_ativo.configure(text="Tempo Ativo: 0.00 s")
...
# Frame destacado para resultado FFT
frame_fft_resultado = ctk.CTkFrame(frame_side, fg_color="#2b5278")
frame_fft_resultado.pack(fill="x", padx=8, pady=5)
lbl_fft_pico = ctk.CTkLabel(frame_fft_resultado, text="FFT: —", font=ctk.CTkFont(size=16, weight="bold"), text_color="#FFD700")
lbl_fft_pico.pack(padx=10, pady=10)

ctk.CTkLabel(frame_side, text="Métricas de Picos", font=ctk.CTkFont(size=14, weight="bold")).pack(fill="x", padx=8, pady=(15, 2))

# Frame destacado para métricas de picos
frame_metricas_picos = ctk.CTkFrame(frame_side, fg_color="#2b5278")
frame_metricas_picos.pack(fill="x", padx=8, pady=5)

lbl_npicos = ctk.CTkLabel(frame_metricas_picos, text="Picos: —", font=ctk.CTkFont(size=14, weight="bold"), text_color="#FFFFFF")
lbl_npicos.pack(fill="x", padx=10, pady=(10,3), anchor="w")

lbl_dur = ctk.CTkLabel(frame_metricas_picos, text="Δt (1º→último): —", font=ctk.CTkFont(size=13), text_color="#CCCCCC")
lbl_dur.pack(fill="x", padx=10, pady=2, anchor="w")

lbl_freq = ctk.CTkLabel(frame_metricas_picos, text="Frequência: —", font=ctk.CTkFont(size=16, weight="bold"), text_color="#87CEEB")
lbl_freq.pack(fill="x", padx=10, pady=(3,10), anchor="w")

# Label de Tempo Ativo (destaque)
ctk.CTkLabel(frame_side, text="Análise de Manobra", font=ctk.CTkFont(size=14, weight="bold")).pack(fill="x", padx=8, pady=(15, 2))
frame_tempo_ativo = ctk.CTkFrame(frame_side, fg_color="#2b5278")
frame_tempo_ativo.pack(fill="x", padx=8, pady=5)
lbl_tempo_total_corte = ctk.CTkLabel(frame_tempo_ativo, text="Tempo Total: 0.00 s", font=ctk.CTkFont(size=14), text_color="#FFFFFF")
lbl_tempo_total_corte.pack(padx=10, pady=(10,5))
lbl_tempo_ativo = ctk.CTkLabel(frame_tempo_ativo, text="Tempo Ativo: 0.00 s", font=ctk.CTkFont(size=16, weight="bold"), text_color="#90EE90")
lbl_tempo_ativo.pack(padx=10, pady=(5,10))
